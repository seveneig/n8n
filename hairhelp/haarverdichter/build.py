#!/usr/bin/env python3
"""
Baut aus haarverdichter.html die fertige Seite nach ausgabe/.

  haarverdichter.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/haarverdichter.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /haarverdichter/ extrahiert.
"""
...
SRC = os.path.join(HERE, "haarverdichter.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in haarverdichter.html")
    ...
    target = os.path.join(DIST, "haarverdichter.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("haarverdichter.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
