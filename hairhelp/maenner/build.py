#!/usr/bin/env python3
"""
Baut aus maenner.html die fertige Seite nach ausgabe/.

  maenner.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/maenner.html Alles eingebettet, läuft ohne Netzwerkzugriff.
...
SRC = os.path.join(HERE, "maenner.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in maenner.html")
    ...
    target = os.path.join(DIST, "maenner.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("maenner.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
