#!/usr/bin/env python3
"""
Baut aus erfahrungen.html die fertige Seite nach ausgabe/.

  erfahrungen.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/erfahrungen.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /streuhaar-erfahrungen/ extrahiert.
"""
...
SRC = os.path.join(HERE, "erfahrungen.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in erfahrungen.html")
    ...
    target = os.path.join(DIST, "erfahrungen.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("erfahrungen.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
