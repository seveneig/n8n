#!/usr/bin/env python3
"""
Baut aus warenkorb.html die fertige Seite nach ausgabe/.

  warenkorb.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/warenkorb.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /wk/ extrahiert.
"""
...
SRC = os.path.join(HERE, "warenkorb.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in warenkorb.html")
    ...
    target = os.path.join(DIST, "warenkorb.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("warenkorb.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
