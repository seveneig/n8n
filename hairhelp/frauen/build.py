#!/usr/bin/env python3
"""
Baut aus frauen.html die fertige Seite nach ausgabe/.

  frauen.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/frauen.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /streuhaar-frauen/ extrahiert.
"""
...
SRC = os.path.join(HERE, "frauen.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in frauen.html")
    ...
    target = os.path.join(DIST, "frauen.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("frauen.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
