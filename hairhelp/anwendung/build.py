#!/usr/bin/env python3
"""
Baut aus anwendung.html die fertige Seite nach ausgabe/.

  anwendung.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/anwendung.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /streuhaar-anwendung/ extrahiert.
"""
...
SRC = os.path.join(HERE, "anwendung.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in anwendung.html")
    ...
    target = os.path.join(DIST, "anwendung.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("anwendung.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
