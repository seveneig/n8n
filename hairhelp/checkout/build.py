#!/usr/bin/env python3
"""
Baut aus checkout.html die fertige Seite nach ausgabe/.

  checkout.html         Quelle. Bilder und Schrift als ASSET:name-Platzhalter.
  ausgabe/checkout.html Alles eingebettet, läuft ohne Netzwerkzugriff.

Die Assets stammen aus assets.json und wurden aus dem gespeicherten
Seiten-Dump von /checkout/ extrahiert.
"""
...
SRC = os.path.join(HERE, "checkout.html")
...
def main():
    ...
    if marker not in html:
        raise SystemExit("Font-Marker fehlt in checkout.html")
    ...
    target = os.path.join(DIST, "checkout.html")
    with open(target, "w", encoding="utf-8") as fh:
        fh.write(html)
    print("checkout.html  %6.2f MB" % (os.path.getsize(target) / 1e6))
